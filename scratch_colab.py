...
#### Data Prep ####
dataset_name = "./drive/MyDrive/transformer_training/trump/trump_replies.csv"
...
#### Hyperparametres ####
model = Transformer(len(vocabulary), maxLength=max_length, embeddingSize=150, numberEncoderLayers=2, numberDecoderLayers=2, attentionHeadCount=2, transformerHiddenDenseSize=128, batch_size=batch_size).cuda()
...
for epoch in range(epochs):
    ...
    batch_data_feed = tqdm.notebook.tqdm(enumerate(zip(inputs_batched, outputs_batched)), total=len(inputs_batched), ncols='100%')
    ...
    with open("./drive/MyDrive/transformer_training/trump/training-log.csv", "a+") as df:
    ...
    }, f'./drive/MyDrive/transformer_training/trump/{modelID}-{checkpointID}.model')
